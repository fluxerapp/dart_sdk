//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_import

import 'package:one_of_serializer/any_of_serializer.dart';
import 'package:one_of_serializer/one_of_serializer.dart';
import 'package:built_collection/built_collection.dart';
import 'package:built_value/json_object.dart';
import 'package:built_value/serializer.dart';
import 'package:built_value/standard_json_plugin.dart';
import 'package:built_value/iso_8601_date_time_serializer.dart';
import 'package:fluxer_dart/src/date_serializer.dart';
import 'package:fluxer_dart/src/model/date.dart';

import 'package:fluxer_dart/src/model/api_error_code.dart';
import 'package:fluxer_dart/src/model/add_snowflake_reservation_request.dart';
import 'package:fluxer_dart/src/model/admin_archive_response_schema.dart';
import 'package:fluxer_dart/src/model/admin_audit_log_response_schema.dart';
import 'package:fluxer_dart/src/model/admin_user_dm_channel_schema.dart';
import 'package:fluxer_dart/src/model/admin_users_me_response.dart';
import 'package:fluxer_dart/src/model/application_bot_response.dart';
import 'package:fluxer_dart/src/model/application_create_request.dart';
import 'package:fluxer_dart/src/model/application_public_response.dart';
import 'package:fluxer_dart/src/model/application_response.dart';
import 'package:fluxer_dart/src/model/application_update_request.dart';
import 'package:fluxer_dart/src/model/applications_me_response.dart';
import 'package:fluxer_dart/src/model/audit_log_action_type.dart';
import 'package:fluxer_dart/src/model/audit_log_change_schema.dart';
import 'package:fluxer_dart/src/model/audit_log_change_schema_new_value.dart';
import 'package:fluxer_dart/src/model/audit_log_change_schema_old_value.dart';
import 'package:fluxer_dart/src/model/audit_log_change_schema_old_value_one_of.dart';
import 'package:fluxer_dart/src/model/audit_log_webhook_response.dart';
import 'package:fluxer_dart/src/model/audit_logs_list_response_schema.dart';
import 'package:fluxer_dart/src/model/auth_login_response.dart';
import 'package:fluxer_dart/src/model/auth_mfa_required_response.dart';
import 'package:fluxer_dart/src/model/auth_register_response.dart';
import 'package:fluxer_dart/src/model/auth_session_client_info.dart';
import 'package:fluxer_dart/src/model/auth_session_location.dart';
import 'package:fluxer_dart/src/model/auth_session_response.dart';
import 'package:fluxer_dart/src/model/auth_token_with_user_id_response.dart';
import 'package:fluxer_dart/src/model/authenticator_type.dart';
import 'package:fluxer_dart/src/model/authorize_consent_request.dart';
import 'package:fluxer_dart/src/model/authorize_ip_request.dart';
import 'package:fluxer_dart/src/model/ban_check_response_schema.dart';
import 'package:fluxer_dart/src/model/ban_email_request.dart';
import 'package:fluxer_dart/src/model/ban_guild_member_request.dart';
import 'package:fluxer_dart/src/model/ban_ip_request.dart';
import 'package:fluxer_dart/src/model/ban_phone_request.dart';
import 'package:fluxer_dart/src/model/bluesky_authorize_request.dart';
import 'package:fluxer_dart/src/model/bluesky_authorize_response.dart';
import 'package:fluxer_dart/src/model/bot_profile_response.dart';
import 'package:fluxer_dart/src/model/bot_profile_update_request.dart';
import 'package:fluxer_dart/src/model/bot_token_reset_response.dart';
import 'package:fluxer_dart/src/model/bulk_add_guild_members_request.dart';
import 'package:fluxer_dart/src/model/bulk_delete_messages_request.dart';
import 'package:fluxer_dart/src/model/bulk_operation_failed_response.dart';
import 'package:fluxer_dart/src/model/bulk_operation_response.dart';
import 'package:fluxer_dart/src/model/bulk_schedule_user_deletion_request.dart';
import 'package:fluxer_dart/src/model/bulk_update_guild_features_request.dart';
import 'package:fluxer_dart/src/model/bulk_update_user_flags_request.dart';
import 'package:fluxer_dart/src/model/call_eligibility_response.dart';
import 'package:fluxer_dart/src/model/call_ring_body_schema.dart';
import 'package:fluxer_dart/src/model/call_update_body_schema.dart';
import 'package:fluxer_dart/src/model/cancel_bulk_message_deletion_request.dart';
import 'package:fluxer_dart/src/model/change_dob_request.dart';
import 'package:fluxer_dart/src/model/change_email_request.dart';
import 'package:fluxer_dart/src/model/change_username_request.dart';
import 'package:fluxer_dart/src/model/channel_create_category_request.dart';
import 'package:fluxer_dart/src/model/channel_create_link_request.dart';
import 'package:fluxer_dart/src/model/channel_create_request.dart';
import 'package:fluxer_dart/src/model/channel_create_text_request.dart';
import 'package:fluxer_dart/src/model/channel_create_voice_request.dart';
import 'package:fluxer_dart/src/model/channel_invite_create_request.dart';
import 'package:fluxer_dart/src/model/channel_nickname_overrides_value.dart';
import 'package:fluxer_dart/src/model/channel_overwrite_request.dart';
import 'package:fluxer_dart/src/model/channel_overwrite_response.dart';
import 'package:fluxer_dart/src/model/channel_partial_recipient_response.dart';
import 'package:fluxer_dart/src/model/channel_partial_response.dart';
import 'package:fluxer_dart/src/model/channel_pin_message_response.dart';
import 'package:fluxer_dart/src/model/channel_pin_response.dart';
import 'package:fluxer_dart/src/model/channel_pins_response.dart';
import 'package:fluxer_dart/src/model/channel_position_update_request_inner.dart';
import 'package:fluxer_dart/src/model/channel_response.dart';
import 'package:fluxer_dart/src/model/channel_update_category_request.dart';
import 'package:fluxer_dart/src/model/channel_update_group_dm_request.dart';
import 'package:fluxer_dart/src/model/channel_update_link_request.dart';
import 'package:fluxer_dart/src/model/channel_update_request.dart';
import 'package:fluxer_dart/src/model/channel_update_text_request.dart';
import 'package:fluxer_dart/src/model/channel_update_voice_request.dart';
import 'package:fluxer_dart/src/model/clear_guild_fields_request.dart';
import 'package:fluxer_dart/src/model/clear_user_fields_request.dart';
import 'package:fluxer_dart/src/model/codes_response.dart';
import 'package:fluxer_dart/src/model/connection_response.dart';
import 'package:fluxer_dart/src/model/connection_verification_response.dart';
import 'package:fluxer_dart/src/model/create_admin_api_key_request.dart';
import 'package:fluxer_dart/src/model/create_admin_api_key_response.dart';
import 'package:fluxer_dart/src/model/create_checkout_session_request.dart';
import 'package:fluxer_dart/src/model/create_connection_request.dart';
import 'package:fluxer_dart/src/model/create_favorite_meme_body_schema.dart';
import 'package:fluxer_dart/src/model/create_favorite_meme_from_url_body_schema.dart';
import 'package:fluxer_dart/src/model/create_private_channel_request.dart';
import 'package:fluxer_dart/src/model/create_system_dm_job_request.dart';
import 'package:fluxer_dart/src/model/create_voice_region_request.dart';
import 'package:fluxer_dart/src/model/create_voice_region_response.dart';
import 'package:fluxer_dart/src/model/create_voice_region_response_region.dart';
import 'package:fluxer_dart/src/model/create_voice_server_request.dart';
import 'package:fluxer_dart/src/model/create_voice_server_response.dart';
import 'package:fluxer_dart/src/model/create_voice_server_response_server.dart';
import 'package:fluxer_dart/src/model/custom_status_payload.dart';
import 'package:fluxer_dart/src/model/custom_status_payload_expires_at.dart';
import 'package:fluxer_dart/src/model/custom_status_response.dart';
import 'package:fluxer_dart/src/model/default_message_notifications.dart';
import 'package:fluxer_dart/src/model/delete_all_user_messages_request.dart';
import 'package:fluxer_dart/src/model/delete_all_user_messages_response.dart';
import 'package:fluxer_dart/src/model/delete_api_key_response.dart';
import 'package:fluxer_dart/src/model/delete_guild_request.dart';
import 'package:fluxer_dart/src/model/delete_message_request.dart';
import 'package:fluxer_dart/src/model/delete_message_response.dart';
import 'package:fluxer_dart/src/model/delete_snowflake_reservation_request.dart';
import 'package:fluxer_dart/src/model/delete_voice_region_request.dart';
import 'package:fluxer_dart/src/model/delete_voice_response.dart';
import 'package:fluxer_dart/src/model/delete_voice_server_request.dart';
import 'package:fluxer_dart/src/model/disable_for_suspicious_activity_request.dart';
import 'package:fluxer_dart/src/model/disable_mfa_request.dart';
import 'package:fluxer_dart/src/model/disable_totp_request.dart';
import 'package:fluxer_dart/src/model/discovery_admin_reject_request.dart';
import 'package:fluxer_dart/src/model/discovery_admin_remove_request.dart';
import 'package:fluxer_dart/src/model/discovery_admin_review_request.dart';
import 'package:fluxer_dart/src/model/discovery_application_patch_request.dart';
import 'package:fluxer_dart/src/model/discovery_application_request.dart';
import 'package:fluxer_dart/src/model/discovery_application_response.dart';
import 'package:fluxer_dart/src/model/discovery_category_response.dart';
import 'package:fluxer_dart/src/model/discovery_guild_list_response.dart';
import 'package:fluxer_dart/src/model/discovery_guild_response.dart';
import 'package:fluxer_dart/src/model/discovery_status_response.dart';
import 'package:fluxer_dart/src/model/donation_checkout_request.dart';
import 'package:fluxer_dart/src/model/donation_checkout_response.dart';
import 'package:fluxer_dart/src/model/donation_request_link_request.dart';
import 'package:fluxer_dart/src/model/download_url_response_schema.dart';
import 'package:fluxer_dart/src/model/dsa_report_email_send_request.dart';
import 'package:fluxer_dart/src/model/dsa_report_email_verify_request.dart';
import 'package:fluxer_dart/src/model/dsa_report_guild_request.dart';
import 'package:fluxer_dart/src/model/dsa_report_message_request.dart';
import 'package:fluxer_dart/src/model/dsa_report_request.dart';
import 'package:fluxer_dart/src/model/dsa_report_user_request.dart';
import 'package:fluxer_dart/src/model/email_change_bounced_request_new_request.dart';
import 'package:fluxer_dart/src/model/email_change_bounced_verify_new_request.dart';
import 'package:fluxer_dart/src/model/email_change_request_new_request.dart';
import 'package:fluxer_dart/src/model/email_change_request_new_response.dart';
import 'package:fluxer_dart/src/model/email_change_start_response.dart';
import 'package:fluxer_dart/src/model/email_change_ticket_request.dart';
import 'package:fluxer_dart/src/model/email_change_verify_new_request.dart';
import 'package:fluxer_dart/src/model/email_change_verify_original_request.dart';
import 'package:fluxer_dart/src/model/email_change_verify_original_response.dart';
import 'package:fluxer_dart/src/model/email_revert_request.dart';
import 'package:fluxer_dart/src/model/email_token_response.dart';
import 'package:fluxer_dart/src/model/embed_author_response.dart';
import 'package:fluxer_dart/src/model/embed_field_response.dart';
import 'package:fluxer_dart/src/model/embed_footer_response.dart';
import 'package:fluxer_dart/src/model/embed_media_response.dart';
import 'package:fluxer_dart/src/model/enable_mfa_totp_request.dart';
import 'package:fluxer_dart/src/model/enabled_toggle_request.dart';
import 'package:fluxer_dart/src/model/error.dart';
import 'package:fluxer_dart/src/model/expand_visionary_slots_request.dart';
import 'package:fluxer_dart/src/model/favorite_meme_response.dart';
import 'package:fluxer_dart/src/model/force_add_user_to_guild_request.dart';
import 'package:fluxer_dart/src/model/forgot_password_request.dart';
import 'package:fluxer_dart/src/model/friend_request_by_tag_request.dart';
import 'package:fluxer_dart/src/model/gateway_bot_response.dart';
import 'package:fluxer_dart/src/model/gateway_bot_response_session_start_limit.dart';
import 'package:fluxer_dart/src/model/generate_gift_codes_request.dart';
import 'package:fluxer_dart/src/model/get_archive_response_schema.dart';
import 'package:fluxer_dart/src/model/get_index_refresh_status_request.dart';
import 'package:fluxer_dart/src/model/get_process_memory_stats_request.dart';
import 'package:fluxer_dart/src/model/get_voice_region_request.dart';
import 'package:fluxer_dart/src/model/get_voice_region_response.dart';
import 'package:fluxer_dart/src/model/get_voice_server_request.dart';
import 'package:fluxer_dart/src/model/get_voice_server_response.dart';
import 'package:fluxer_dart/src/model/get_well_known_fluxer429_response.dart';
import 'package:fluxer_dart/src/model/gift_code_metadata_response.dart';
import 'package:fluxer_dart/src/model/gift_code_response.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_answer.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_answer_user.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_check_run.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_check_run_check_suite.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_check_run_check_suite_app.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_check_run_check_suite_pull_requests_inner.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_check_run_output.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_check_suite.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_commits_inner.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_commits_inner_author.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_discussion.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_forkee.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_head_commit.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_issue.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_member.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_release.dart';
import 'package:fluxer_dart/src/model/git_hub_webhook_review.dart';
import 'package:fluxer_dart/src/model/global_search_messages_request.dart';
import 'package:fluxer_dart/src/model/group_dm_invite_metadata_response.dart';
import 'package:fluxer_dart/src/model/group_dm_invite_response.dart';
import 'package:fluxer_dart/src/model/guild_admin_response.dart';
import 'package:fluxer_dart/src/model/guild_asset_item_schema.dart';
import 'package:fluxer_dart/src/model/guild_audit_log_entry_response.dart';
import 'package:fluxer_dart/src/model/guild_audit_log_entry_response_options.dart';
import 'package:fluxer_dart/src/model/guild_audit_log_list_response.dart';
import 'package:fluxer_dart/src/model/guild_ban_create_request.dart';
import 'package:fluxer_dart/src/model/guild_ban_response.dart';
import 'package:fluxer_dart/src/model/guild_create_request.dart';
import 'package:fluxer_dart/src/model/guild_delete_request.dart';
import 'package:fluxer_dart/src/model/guild_emoji_bulk_create_request.dart';
import 'package:fluxer_dart/src/model/guild_emoji_bulk_create_response.dart';
import 'package:fluxer_dart/src/model/guild_emoji_bulk_create_response_failed_inner.dart';
import 'package:fluxer_dart/src/model/guild_emoji_create_request.dart';
import 'package:fluxer_dart/src/model/guild_emoji_response.dart';
import 'package:fluxer_dart/src/model/guild_emoji_update_request.dart';
import 'package:fluxer_dart/src/model/guild_emoji_with_user_response.dart';
import 'package:fluxer_dart/src/model/guild_explicit_content_filter.dart';
import 'package:fluxer_dart/src/model/guild_invite_metadata_response.dart';
import 'package:fluxer_dart/src/model/guild_invite_metadata_response_guild.dart';
import 'package:fluxer_dart/src/model/guild_invite_response.dart';
import 'package:fluxer_dart/src/model/guild_mfa_level.dart';
import 'package:fluxer_dart/src/model/guild_member_response.dart';
import 'package:fluxer_dart/src/model/guild_member_search_request.dart';
import 'package:fluxer_dart/src/model/guild_member_search_response.dart';
import 'package:fluxer_dart/src/model/guild_member_search_result.dart';
import 'package:fluxer_dart/src/model/guild_member_search_result_supplemental.dart';
import 'package:fluxer_dart/src/model/guild_member_update_request.dart';
import 'package:fluxer_dart/src/model/guild_memory_stats_response.dart';
import 'package:fluxer_dart/src/model/guild_memory_stats_response_guilds_inner.dart';
import 'package:fluxer_dart/src/model/guild_report_category_enum.dart';
import 'package:fluxer_dart/src/model/guild_response.dart';
import 'package:fluxer_dart/src/model/guild_role_create_request.dart';
import 'package:fluxer_dart/src/model/guild_role_hoist_position_item.dart';
import 'package:fluxer_dart/src/model/guild_role_position_item.dart';
import 'package:fluxer_dart/src/model/guild_role_response.dart';
import 'package:fluxer_dart/src/model/guild_role_update_request.dart';
import 'package:fluxer_dart/src/model/guild_sticker_bulk_create_request.dart';
import 'package:fluxer_dart/src/model/guild_sticker_bulk_create_response.dart';
import 'package:fluxer_dart/src/model/guild_sticker_bulk_create_response_failed_inner.dart';
import 'package:fluxer_dart/src/model/guild_sticker_create_request.dart';
import 'package:fluxer_dart/src/model/guild_sticker_response.dart';
import 'package:fluxer_dart/src/model/guild_sticker_update_request.dart';
import 'package:fluxer_dart/src/model/guild_sticker_with_user_response.dart';
import 'package:fluxer_dart/src/model/guild_transfer_ownership_request.dart';
import 'package:fluxer_dart/src/model/guild_update_request.dart';
import 'package:fluxer_dart/src/model/guild_update_response.dart';
import 'package:fluxer_dart/src/model/guild_update_response_guild.dart';
import 'package:fluxer_dart/src/model/guild_vanity_url_response.dart';
import 'package:fluxer_dart/src/model/guild_vanity_url_update_request.dart';
import 'package:fluxer_dart/src/model/guild_vanity_url_update_response.dart';
import 'package:fluxer_dart/src/model/guild_verification_level.dart';
import 'package:fluxer_dart/src/model/handoff_complete_request.dart';
import 'package:fluxer_dart/src/model/handoff_initiate_response.dart';
import 'package:fluxer_dart/src/model/handoff_status_response.dart';
import 'package:fluxer_dart/src/model/harvest_creation_response_schema.dart';
import 'package:fluxer_dart/src/model/harvest_download_url_response.dart';
import 'package:fluxer_dart/src/model/harvest_status_enum.dart';
import 'package:fluxer_dart/src/model/harvest_status_response_schema.dart';
import 'package:fluxer_dart/src/model/index_refresh_status_response.dart';
import 'package:fluxer_dart/src/model/index_refresh_status_response_one_of.dart';
import 'package:fluxer_dart/src/model/index_refresh_status_response_one_of1.dart';
import 'package:fluxer_dart/src/model/instance_config_response.dart';
import 'package:fluxer_dart/src/model/instance_config_update_request.dart';
import 'package:fluxer_dart/src/model/instance_config_update_request_sso.dart';
import 'package:fluxer_dart/src/model/invite_metadata_response_schema.dart';
import 'package:fluxer_dart/src/model/invite_response_schema.dart';
import 'package:fluxer_dart/src/model/ip_authorization_poll_response.dart';
import 'package:fluxer_dart/src/model/ip_ban_list_entry_schema.dart';
import 'package:fluxer_dart/src/model/join_source_type.dart';
import 'package:fluxer_dart/src/model/kick_guild_member_request.dart';
import 'package:fluxer_dart/src/model/klipy_category_tag_response.dart';
import 'package:fluxer_dart/src/model/klipy_featured_response.dart';
import 'package:fluxer_dart/src/model/klipy_gif_response.dart';
import 'package:fluxer_dart/src/model/klipy_register_share_request.dart';
import 'package:fluxer_dart/src/model/legal_hold_request.dart';
import 'package:fluxer_dart/src/model/legal_hold_response.dart';
import 'package:fluxer_dart/src/model/limit_config_get_response.dart';
import 'package:fluxer_dart/src/model/limit_config_get_response_bounds_value.dart';
import 'package:fluxer_dart/src/model/limit_config_get_response_limit_config.dart';
import 'package:fluxer_dart/src/model/limit_config_get_response_limit_config_rules_inner.dart';
import 'package:fluxer_dart/src/model/limit_config_get_response_limit_config_rules_inner_filters.dart';
import 'package:fluxer_dart/src/model/limit_config_get_response_metadata_value.dart';
import 'package:fluxer_dart/src/model/limit_config_update_request.dart';
import 'package:fluxer_dart/src/model/limit_config_update_request_limit_config.dart';
import 'package:fluxer_dart/src/model/limit_config_update_request_limit_config_rules_inner.dart';
import 'package:fluxer_dart/src/model/limit_filter_response.dart';
import 'package:fluxer_dart/src/model/limit_rule_response.dart';
import 'package:fluxer_dart/src/model/list_admin_api_key_response.dart';
import 'package:fluxer_dart/src/model/list_archives_request.dart';
import 'package:fluxer_dart/src/model/list_archives_response_schema.dart';
import 'package:fluxer_dart/src/model/list_audit_logs_request.dart';
import 'package:fluxer_dart/src/model/list_bans_request.dart';
import 'package:fluxer_dart/src/model/list_email_bans_response_schema.dart';
import 'package:fluxer_dart/src/model/list_guild_emojis_response.dart';
import 'package:fluxer_dart/src/model/list_guild_members_request.dart';
import 'package:fluxer_dart/src/model/list_guild_members_response.dart';
import 'package:fluxer_dart/src/model/list_guild_stickers_response.dart';
import 'package:fluxer_dart/src/model/list_ip_bans_response_schema.dart';
import 'package:fluxer_dart/src/model/list_phone_bans_response_schema.dart';
import 'package:fluxer_dart/src/model/list_reports_request.dart';
import 'package:fluxer_dart/src/model/list_reports_response.dart';
import 'package:fluxer_dart/src/model/list_snowflake_reservations_response.dart';
import 'package:fluxer_dart/src/model/list_system_dm_jobs_response.dart';
import 'package:fluxer_dart/src/model/list_user_change_log_request.dart';
import 'package:fluxer_dart/src/model/list_user_change_log_response_schema.dart';
import 'package:fluxer_dart/src/model/list_user_dm_channels_request.dart';
import 'package:fluxer_dart/src/model/list_user_dm_channels_response.dart';
import 'package:fluxer_dart/src/model/list_user_guilds_request.dart';
import 'package:fluxer_dart/src/model/list_user_guilds_response.dart';
import 'package:fluxer_dart/src/model/list_user_sessions_request.dart';
import 'package:fluxer_dart/src/model/list_user_sessions_response.dart';
import 'package:fluxer_dart/src/model/list_visionary_slots_response.dart';
import 'package:fluxer_dart/src/model/list_voice_regions_request.dart';
import 'package:fluxer_dart/src/model/list_voice_regions_response.dart';
import 'package:fluxer_dart/src/model/list_voice_servers_request.dart';
import 'package:fluxer_dart/src/model/list_voice_servers_response.dart';
import 'package:fluxer_dart/src/model/locale.dart';
import 'package:fluxer_dart/src/model/login_request.dart';
import 'package:fluxer_dart/src/model/logout_auth_sessions_request.dart';
import 'package:fluxer_dart/src/model/lookup_guild_request.dart';
import 'package:fluxer_dart/src/model/lookup_guild_response.dart';
import 'package:fluxer_dart/src/model/lookup_guild_response_guild.dart';
import 'package:fluxer_dart/src/model/lookup_guild_response_guild_channels_inner.dart';
import 'package:fluxer_dart/src/model/lookup_guild_response_guild_roles_inner.dart';
import 'package:fluxer_dart/src/model/lookup_message_by_attachment_request.dart';
import 'package:fluxer_dart/src/model/lookup_message_request.dart';
import 'package:fluxer_dart/src/model/lookup_message_response.dart';
import 'package:fluxer_dart/src/model/lookup_message_response_messages_inner.dart';
import 'package:fluxer_dart/src/model/lookup_message_response_messages_inner_attachments_inner.dart';
import 'package:fluxer_dart/src/model/lookup_user_by_ids_request.dart';
import 'package:fluxer_dart/src/model/lookup_user_by_query_request.dart';
import 'package:fluxer_dart/src/model/lookup_user_request.dart';
import 'package:fluxer_dart/src/model/lookup_user_response.dart';
import 'package:fluxer_dart/src/model/message_ack_request.dart';
import 'package:fluxer_dart/src/model/message_attachment_response.dart';
import 'package:fluxer_dart/src/model/message_author_type.dart';
import 'package:fluxer_dart/src/model/message_base_response_schema.dart';
import 'package:fluxer_dart/src/model/message_call_response.dart';
import 'package:fluxer_dart/src/model/message_content_type.dart';
import 'package:fluxer_dart/src/model/message_embed_child_response.dart';
import 'package:fluxer_dart/src/model/message_embed_response.dart';
import 'package:fluxer_dart/src/model/message_embed_type.dart';
import 'package:fluxer_dart/src/model/message_reaction_response.dart';
import 'package:fluxer_dart/src/model/message_reaction_response_emoji.dart';
import 'package:fluxer_dart/src/model/message_reference_response.dart';
import 'package:fluxer_dart/src/model/message_reference_type.dart';
import 'package:fluxer_dart/src/model/message_report_category_enum.dart';
import 'package:fluxer_dart/src/model/message_response_schema.dart';
import 'package:fluxer_dart/src/model/message_search_indexing_response.dart';
import 'package:fluxer_dart/src/model/message_search_response.dart';
import 'package:fluxer_dart/src/model/message_search_results_response.dart';
import 'package:fluxer_dart/src/model/message_search_results_response_messages_inner.dart';
import 'package:fluxer_dart/src/model/message_search_scope.dart';
import 'package:fluxer_dart/src/model/message_shred_request.dart';
import 'package:fluxer_dart/src/model/message_shred_request_entries_inner.dart';
import 'package:fluxer_dart/src/model/message_shred_response.dart';
import 'package:fluxer_dart/src/model/message_shred_status_not_found_response.dart';
import 'package:fluxer_dart/src/model/message_shred_status_progress_response.dart';
import 'package:fluxer_dart/src/model/message_shred_status_request.dart';
import 'package:fluxer_dart/src/model/message_shred_status_response.dart';
import 'package:fluxer_dart/src/model/message_snapshot_response.dart';
import 'package:fluxer_dart/src/model/message_sort_field.dart';
import 'package:fluxer_dart/src/model/message_sort_order.dart';
import 'package:fluxer_dart/src/model/message_sticker_response.dart';
import 'package:fluxer_dart/src/model/mfa_backup_code_response.dart';
import 'package:fluxer_dart/src/model/mfa_backup_codes_request.dart';
import 'package:fluxer_dart/src/model/mfa_backup_codes_response.dart';
import 'package:fluxer_dart/src/model/mfa_sms_request.dart';
import 'package:fluxer_dart/src/model/mfa_ticket_request.dart';
import 'package:fluxer_dart/src/model/mfa_totp_request.dart';
import 'package:fluxer_dart/src/model/mutual_guild_response.dart';
import 'package:fluxer_dart/src/model/my_guild_member_update_request.dart';
import 'package:fluxer_dart/src/model/nsfw_level.dart';
import 'package:fluxer_dart/src/model/ncmec_submission_status_response.dart';
import 'package:fluxer_dart/src/model/ncmec_submit_result_response.dart';
import 'package:fluxer_dart/src/model/node_stats_response.dart';
import 'package:fluxer_dart/src/model/node_stats_response_memory.dart';
import 'package:fluxer_dart/src/model/o_auth2_authorization_response.dart';
import 'package:fluxer_dart/src/model/o_auth2_authorization_response_application.dart';
import 'package:fluxer_dart/src/model/o_auth2_consent_response.dart';
import 'package:fluxer_dart/src/model/o_auth2_introspect_response.dart';
import 'package:fluxer_dart/src/model/o_auth2_me_response.dart';
import 'package:fluxer_dart/src/model/o_auth2_me_response_application.dart';
import 'package:fluxer_dart/src/model/o_auth2_me_response_user.dart';
import 'package:fluxer_dart/src/model/o_auth2_token_response.dart';
import 'package:fluxer_dart/src/model/o_auth2_user_info_response.dart';
import 'package:fluxer_dart/src/model/ok_response.dart';
import 'package:fluxer_dart/src/model/pack_create_request.dart';
import 'package:fluxer_dart/src/model/pack_dashboard_response.dart';
import 'package:fluxer_dart/src/model/pack_dashboard_response_emoji.dart';
import 'package:fluxer_dart/src/model/pack_dashboard_response_sticker.dart';
import 'package:fluxer_dart/src/model/pack_invite_create_request.dart';
import 'package:fluxer_dart/src/model/pack_invite_metadata_response.dart';
import 'package:fluxer_dart/src/model/pack_invite_metadata_response_pack.dart';
import 'package:fluxer_dart/src/model/pack_invite_response.dart';
import 'package:fluxer_dart/src/model/pack_summary_response.dart';
import 'package:fluxer_dart/src/model/pack_update_request.dart';
import 'package:fluxer_dart/src/model/password_change_complete_request.dart';
import 'package:fluxer_dart/src/model/password_change_start_response.dart';
import 'package:fluxer_dart/src/model/password_change_ticket_request.dart';
import 'package:fluxer_dart/src/model/password_change_verify_request.dart';
import 'package:fluxer_dart/src/model/password_change_verify_response.dart';
import 'package:fluxer_dart/src/model/permission_overwrite_create_request.dart';
import 'package:fluxer_dart/src/model/phone_add_request.dart';
import 'package:fluxer_dart/src/model/phone_send_verification_request.dart';
import 'package:fluxer_dart/src/model/phone_verify_request.dart';
import 'package:fluxer_dart/src/model/phone_verify_response.dart';
import 'package:fluxer_dart/src/model/preload_messages_request.dart';
import 'package:fluxer_dart/src/model/preload_messages_response_value.dart';
import 'package:fluxer_dart/src/model/price_ids_response.dart';
import 'package:fluxer_dart/src/model/purge_guild_asset_error_schema.dart';
import 'package:fluxer_dart/src/model/purge_guild_asset_result_schema.dart';
import 'package:fluxer_dart/src/model/purge_guild_assets_request.dart';
import 'package:fluxer_dart/src/model/purge_guild_assets_response_schema.dart';
import 'package:fluxer_dart/src/model/push_subscribe_request.dart';
import 'package:fluxer_dart/src/model/push_subscribe_request_keys.dart';
import 'package:fluxer_dart/src/model/push_subscribe_response.dart';
import 'package:fluxer_dart/src/model/push_subscription_item_response.dart';
import 'package:fluxer_dart/src/model/push_subscriptions_list_response.dart';
import 'package:fluxer_dart/src/model/read_state_ack_bulk_request.dart';
import 'package:fluxer_dart/src/model/refresh_search_index_request.dart';
import 'package:fluxer_dart/src/model/refresh_search_index_response.dart';
import 'package:fluxer_dart/src/model/register_request.dart';
import 'package:fluxer_dart/src/model/relationship_nickname_update_request.dart';
import 'package:fluxer_dart/src/model/relationship_response.dart';
import 'package:fluxer_dart/src/model/relationship_type_put_request.dart';
import 'package:fluxer_dart/src/model/relationship_types.dart';
import 'package:fluxer_dart/src/model/reload_all_guilds_response.dart';
import 'package:fluxer_dart/src/model/reload_guild_request.dart';
import 'package:fluxer_dart/src/model/reload_guilds_request.dart';
import 'package:fluxer_dart/src/model/render_spoilers.dart';
import 'package:fluxer_dart/src/model/reorder_connections_request.dart';
import 'package:fluxer_dart/src/model/report_admin_response_schema.dart';
import 'package:fluxer_dart/src/model/report_admin_response_schema_message_context_inner.dart';
import 'package:fluxer_dart/src/model/report_guild_request.dart';
import 'package:fluxer_dart/src/model/report_message_request.dart';
import 'package:fluxer_dart/src/model/report_response.dart';
import 'package:fluxer_dart/src/model/report_status.dart';
import 'package:fluxer_dart/src/model/report_type.dart';
import 'package:fluxer_dart/src/model/report_user_request.dart';
import 'package:fluxer_dart/src/model/reserve_visionary_slot_request.dart';
import 'package:fluxer_dart/src/model/reset_password_request.dart';
import 'package:fluxer_dart/src/model/resolve_report_request.dart';
import 'package:fluxer_dart/src/model/resolve_report_response.dart';
import 'package:fluxer_dart/src/model/rtc_region_response.dart';
import 'package:fluxer_dart/src/model/save_message_request.dart';
import 'package:fluxer_dart/src/model/saved_message_entry_response.dart';
import 'package:fluxer_dart/src/model/schedule_account_deletion_request.dart';
import 'package:fluxer_dart/src/model/scheduled_message_allowed_mentions_schema.dart';
import 'package:fluxer_dart/src/model/scheduled_message_reference_schema.dart';
import 'package:fluxer_dart/src/model/scheduled_message_response_schema.dart';
import 'package:fluxer_dart/src/model/scheduled_message_response_schema_payload.dart';
import 'package:fluxer_dart/src/model/search_audit_logs_request.dart';
import 'package:fluxer_dart/src/model/search_guilds_request.dart';
import 'package:fluxer_dart/src/model/search_guilds_response.dart';
import 'package:fluxer_dart/src/model/search_reports_request.dart';
import 'package:fluxer_dart/src/model/search_reports_response.dart';
import 'package:fluxer_dart/src/model/search_users_request.dart';
import 'package:fluxer_dart/src/model/search_users_response.dart';
import 'package:fluxer_dart/src/model/send_password_reset_request.dart';
import 'package:fluxer_dart/src/model/sentry_webhook.dart';
import 'package:fluxer_dart/src/model/sentry_webhook_actor.dart';
import 'package:fluxer_dart/src/model/sentry_webhook_data.dart';
import 'package:fluxer_dart/src/model/sentry_webhook_data_issue.dart';
import 'package:fluxer_dart/src/model/sentry_webhook_data_issue_metadata.dart';
import 'package:fluxer_dart/src/model/sentry_webhook_data_issue_project.dart';
import 'package:fluxer_dart/src/model/sentry_webhook_installation.dart';
import 'package:fluxer_dart/src/model/set_user_acls_request.dart';
import 'package:fluxer_dart/src/model/set_user_bot_status_request.dart';
import 'package:fluxer_dart/src/model/set_user_system_status_request.dart';
import 'package:fluxer_dart/src/model/set_user_traits_request.dart';
import 'package:fluxer_dart/src/model/shrink_visionary_slots_request.dart';
import 'package:fluxer_dart/src/model/shutdown_guild_request.dart';
import 'package:fluxer_dart/src/model/slack_webhook_request.dart';
import 'package:fluxer_dart/src/model/slack_webhook_request_attachments_inner.dart';
import 'package:fluxer_dart/src/model/slack_webhook_request_attachments_inner_fields_inner.dart';
import 'package:fluxer_dart/src/model/snowflake_reservation_entry.dart';
import 'package:fluxer_dart/src/model/sso_complete_request.dart';
import 'package:fluxer_dart/src/model/sso_complete_response.dart';
import 'package:fluxer_dart/src/model/sso_config_response.dart';
import 'package:fluxer_dart/src/model/sso_start_request.dart';
import 'package:fluxer_dart/src/model/sso_start_response.dart';
import 'package:fluxer_dart/src/model/sso_status_response.dart';
import 'package:fluxer_dart/src/model/sticker_animation_options.dart';
import 'package:fluxer_dart/src/model/stream_preview_upload_body_schema.dart';
import 'package:fluxer_dart/src/model/stream_update_body_schema.dart';
import 'package:fluxer_dart/src/model/success_response.dart';
import 'package:fluxer_dart/src/model/sudo_mfa_methods_response.dart';
import 'package:fluxer_dart/src/model/sudo_verification_schema.dart';
import 'package:fluxer_dart/src/model/swap_visionary_slots_request.dart';
import 'package:fluxer_dart/src/model/system_dm_job_response.dart';
import 'package:fluxer_dart/src/model/temp_ban_user_request.dart';
import 'package:fluxer_dart/src/model/tenor_category_tag_response.dart';
import 'package:fluxer_dart/src/model/tenor_featured_response.dart';
import 'package:fluxer_dart/src/model/tenor_gif_response.dart';
import 'package:fluxer_dart/src/model/tenor_register_share_request.dart';
import 'package:fluxer_dart/src/model/terminate_sessions_request.dart';
import 'package:fluxer_dart/src/model/terminate_sessions_response.dart';
import 'package:fluxer_dart/src/model/theme_create_request.dart';
import 'package:fluxer_dart/src/model/theme_create_response.dart';
import 'package:fluxer_dart/src/model/ticket_response.dart';
import 'package:fluxer_dart/src/model/time_format_types.dart';
import 'package:fluxer_dart/src/model/transfer_guild_ownership_request.dart';
import 'package:fluxer_dart/src/model/trigger_guild_archive_request.dart';
import 'package:fluxer_dart/src/model/trigger_user_archive_request.dart';
import 'package:fluxer_dart/src/model/unlink_phone_request.dart';
import 'package:fluxer_dart/src/model/update_connection_request.dart';
import 'package:fluxer_dart/src/model/update_favorite_meme_body_schema.dart';
import 'package:fluxer_dart/src/model/update_guild_features_request.dart';
import 'package:fluxer_dart/src/model/update_guild_name_request.dart';
import 'package:fluxer_dart/src/model/update_guild_settings_request.dart';
import 'package:fluxer_dart/src/model/update_guild_vanity_request.dart';
import 'package:fluxer_dart/src/model/update_suspicious_activity_flags_request.dart';
import 'package:fluxer_dart/src/model/update_user_flags_request.dart';
import 'package:fluxer_dart/src/model/update_voice_region_request.dart';
import 'package:fluxer_dart/src/model/update_voice_region_response.dart';
import 'package:fluxer_dart/src/model/update_voice_region_response_region.dart';
import 'package:fluxer_dart/src/model/update_voice_server_request.dart';
import 'package:fluxer_dart/src/model/update_voice_server_response.dart';
import 'package:fluxer_dart/src/model/update_voice_server_response_server.dart';
import 'package:fluxer_dart/src/model/url_response.dart';
import 'package:fluxer_dart/src/model/user_admin_response_schema.dart';
import 'package:fluxer_dart/src/model/user_authenticator_types.dart';
import 'package:fluxer_dart/src/model/user_contact_change_log_entry_schema.dart';
import 'package:fluxer_dart/src/model/user_guild_settings_response.dart';
import 'package:fluxer_dart/src/model/user_guild_settings_response_channel_overrides_value.dart';
import 'package:fluxer_dart/src/model/user_guild_settings_response_mute_config.dart';
import 'package:fluxer_dart/src/model/user_guild_settings_update_request.dart';
import 'package:fluxer_dart/src/model/user_guild_settings_update_request_channel_overrides_value.dart';
import 'package:fluxer_dart/src/model/user_guild_settings_update_request_mute_config.dart';
import 'package:fluxer_dart/src/model/user_guild_settings_update_request_mute_config_end_time.dart';
import 'package:fluxer_dart/src/model/user_mutation_response.dart';
import 'package:fluxer_dart/src/model/user_note_response.dart';
import 'package:fluxer_dart/src/model/user_note_update_request.dart';
import 'package:fluxer_dart/src/model/user_notification_settings.dart';
import 'package:fluxer_dart/src/model/user_partial_response.dart';
import 'package:fluxer_dart/src/model/user_premium_types.dart';
import 'package:fluxer_dart/src/model/user_private_response.dart';
import 'package:fluxer_dart/src/model/user_private_response_pending_bulk_message_deletion.dart';
import 'package:fluxer_dart/src/model/user_profile_full_response.dart';
import 'package:fluxer_dart/src/model/user_profile_full_response_guild_member_profile.dart';
import 'package:fluxer_dart/src/model/user_profile_full_response_user.dart';
import 'package:fluxer_dart/src/model/user_profile_full_response_user_profile.dart';
import 'package:fluxer_dart/src/model/user_report_category_enum.dart';
import 'package:fluxer_dart/src/model/user_session_response.dart';
import 'package:fluxer_dart/src/model/user_settings_response.dart';
import 'package:fluxer_dart/src/model/user_settings_response_guild_folders_inner.dart';
import 'package:fluxer_dart/src/model/user_settings_update_request.dart';
import 'package:fluxer_dart/src/model/user_settings_update_request_guild_folders_inner.dart';
import 'package:fluxer_dart/src/model/user_settings_update_request_status_resets_at.dart';
import 'package:fluxer_dart/src/model/user_status_type.dart';
import 'package:fluxer_dart/src/model/user_tag_check_response.dart';
import 'package:fluxer_dart/src/model/user_theme_type.dart';
import 'package:fluxer_dart/src/model/user_update_with_verification_request.dart';
import 'package:fluxer_dart/src/model/username_suggestions_request.dart';
import 'package:fluxer_dart/src/model/username_suggestions_response.dart';
import 'package:fluxer_dart/src/model/validation_error_item.dart';
import 'package:fluxer_dart/src/model/verify_and_create_connection_request.dart';
import 'package:fluxer_dart/src/model/verify_email_request.dart';
import 'package:fluxer_dart/src/model/verify_user_email_request.dart';
import 'package:fluxer_dart/src/model/visionary_slot_operation_response.dart';
import 'package:fluxer_dart/src/model/visionary_slot_schema.dart';
import 'package:fluxer_dart/src/model/voice_region_admin_response.dart';
import 'package:fluxer_dart/src/model/voice_region_with_servers_response.dart';
import 'package:fluxer_dart/src/model/voice_server_admin_response.dart';
import 'package:fluxer_dart/src/model/web_authn_authenticate_request.dart';
import 'package:fluxer_dart/src/model/web_authn_challenge_response.dart';
import 'package:fluxer_dart/src/model/web_authn_credential_response.dart';
import 'package:fluxer_dart/src/model/web_authn_credential_update_request.dart';
import 'package:fluxer_dart/src/model/web_authn_mfa_request.dart';
import 'package:fluxer_dart/src/model/web_authn_register_request.dart';
import 'package:fluxer_dart/src/model/webhook_create_request.dart';
import 'package:fluxer_dart/src/model/webhook_received_response.dart';
import 'package:fluxer_dart/src/model/webhook_response.dart';
import 'package:fluxer_dart/src/model/webhook_token_response.dart';
import 'package:fluxer_dart/src/model/webhook_token_update_request.dart';
import 'package:fluxer_dart/src/model/webhook_type.dart';
import 'package:fluxer_dart/src/model/webhook_update_request.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_app_public.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_captcha.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_endpoints.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_features.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_federation.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_gif.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_limits.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_oauth2.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_public_key.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_push.dart';
import 'package:fluxer_dart/src/model/well_known_fluxer_response_sso.dart';

part 'serializers.g.dart';

@SerializersFor([
  APIErrorCode,
  AddSnowflakeReservationRequest,
  AdminArchiveResponseSchema,
  AdminAuditLogResponseSchema,
  AdminUserDmChannelSchema,
  AdminUsersMeResponse,
  ApplicationBotResponse,
  ApplicationCreateRequest,
  ApplicationPublicResponse,
  ApplicationResponse,
  ApplicationUpdateRequest,
  ApplicationsMeResponse,
  AuditLogActionType,
  AuditLogChangeSchema,
  AuditLogChangeSchemaNewValue,
  AuditLogChangeSchemaOldValue,
  AuditLogChangeSchemaOldValueOneOf,
  AuditLogWebhookResponse,
  AuditLogsListResponseSchema,
  AuthLoginResponse,
  AuthMfaRequiredResponse,
  AuthRegisterResponse,
  AuthSessionClientInfo,
  AuthSessionLocation,
  AuthSessionResponse,
  AuthTokenWithUserIdResponse,
  AuthenticatorType,
  AuthorizeConsentRequest,
  AuthorizeIpRequest,
  BanCheckResponseSchema,
  BanEmailRequest,
  BanGuildMemberRequest,
  BanIpRequest,
  BanPhoneRequest,
  BlueskyAuthorizeRequest,
  BlueskyAuthorizeResponse,
  BotProfileResponse,
  BotProfileUpdateRequest,
  BotTokenResetResponse,
  BulkAddGuildMembersRequest,
  BulkDeleteMessagesRequest,
  BulkOperationFailedResponse,
  BulkOperationResponse,
  BulkScheduleUserDeletionRequest,
  BulkUpdateGuildFeaturesRequest,
  BulkUpdateUserFlagsRequest,
  CallEligibilityResponse,
  CallRingBodySchema,
  CallUpdateBodySchema,
  CancelBulkMessageDeletionRequest,
  ChangeDobRequest,
  ChangeEmailRequest,
  ChangeUsernameRequest,
  ChannelCreateCategoryRequest,
  ChannelCreateLinkRequest,
  ChannelCreateRequest,
  ChannelCreateTextRequest,
  ChannelCreateVoiceRequest,
  ChannelInviteCreateRequest,
  ChannelNicknameOverridesValue,
  ChannelOverwriteRequest,
  ChannelOverwriteResponse,
  ChannelPartialRecipientResponse,
  ChannelPartialResponse,
  ChannelPinMessageResponse,
  ChannelPinResponse,
  ChannelPinsResponse,
  ChannelPositionUpdateRequestInner,
  ChannelResponse,
  ChannelUpdateCategoryRequest,
  ChannelUpdateGroupDmRequest,
  ChannelUpdateLinkRequest,
  ChannelUpdateRequest,
  ChannelUpdateTextRequest,
  ChannelUpdateVoiceRequest,
  ClearGuildFieldsRequest,
  ClearUserFieldsRequest,
  CodesResponse,
  ConnectionResponse,
  ConnectionVerificationResponse,
  CreateAdminApiKeyRequest,
  CreateAdminApiKeyResponse,
  CreateCheckoutSessionRequest,
  CreateConnectionRequest,
  CreateFavoriteMemeBodySchema,
  CreateFavoriteMemeFromUrlBodySchema,
  CreatePrivateChannelRequest,
  CreateSystemDmJobRequest,
  CreateVoiceRegionRequest,
  CreateVoiceRegionResponse,
  CreateVoiceRegionResponseRegion,
  CreateVoiceServerRequest,
  CreateVoiceServerResponse,
  CreateVoiceServerResponseServer,
  CustomStatusPayload,
  CustomStatusPayloadExpiresAt,
  CustomStatusResponse,
  DefaultMessageNotifications,
  DeleteAllUserMessagesRequest,
  DeleteAllUserMessagesResponse,
  DeleteApiKeyResponse,
  DeleteGuildRequest,
  DeleteMessageRequest,
  DeleteMessageResponse,
  DeleteSnowflakeReservationRequest,
  DeleteVoiceRegionRequest,
  DeleteVoiceResponse,
  DeleteVoiceServerRequest,
  DisableForSuspiciousActivityRequest,
  DisableMfaRequest,
  DisableTotpRequest,
  DiscoveryAdminRejectRequest,
  DiscoveryAdminRemoveRequest,
  DiscoveryAdminReviewRequest,
  DiscoveryApplicationPatchRequest,
  DiscoveryApplicationRequest,
  DiscoveryApplicationResponse,
  DiscoveryCategoryResponse,
  DiscoveryGuildListResponse,
  DiscoveryGuildResponse,
  DiscoveryStatusResponse,
  DonationCheckoutRequest,
  DonationCheckoutResponse,
  DonationRequestLinkRequest,
  DownloadUrlResponseSchema,
  DsaReportEmailSendRequest,
  DsaReportEmailVerifyRequest,
  DsaReportGuildRequest,
  DsaReportMessageRequest,
  DsaReportRequest,
  DsaReportUserRequest,
  EmailChangeBouncedRequestNewRequest,
  EmailChangeBouncedVerifyNewRequest,
  EmailChangeRequestNewRequest,
  EmailChangeRequestNewResponse,
  EmailChangeStartResponse,
  EmailChangeTicketRequest,
  EmailChangeVerifyNewRequest,
  EmailChangeVerifyOriginalRequest,
  EmailChangeVerifyOriginalResponse,
  EmailRevertRequest,
  EmailTokenResponse,
  EmbedAuthorResponse,
  EmbedFieldResponse,
  EmbedFooterResponse,
  EmbedMediaResponse,
  EnableMfaTotpRequest,
  EnabledToggleRequest,
  Error,
  ExpandVisionarySlotsRequest,
  FavoriteMemeResponse,
  ForceAddUserToGuildRequest,
  ForgotPasswordRequest,
  FriendRequestByTagRequest,
  GatewayBotResponse,
  GatewayBotResponseSessionStartLimit,
  GenerateGiftCodesRequest,
  GetArchiveResponseSchema,
  GetIndexRefreshStatusRequest,
  GetProcessMemoryStatsRequest,
  GetVoiceRegionRequest,
  GetVoiceRegionResponse,
  GetVoiceServerRequest,
  GetVoiceServerResponse,
  GetWellKnownFluxer429Response,
  GiftCodeMetadataResponse,
  GiftCodeResponse,
  GitHubWebhook,
  GitHubWebhookAnswer,
  GitHubWebhookAnswerUser,
  GitHubWebhookCheckRun,
  GitHubWebhookCheckRunCheckSuite,
  GitHubWebhookCheckRunCheckSuiteApp,
  GitHubWebhookCheckRunCheckSuitePullRequestsInner,
  GitHubWebhookCheckRunOutput,
  GitHubWebhookCheckSuite,
  GitHubWebhookCommitsInner,
  GitHubWebhookCommitsInnerAuthor,
  GitHubWebhookDiscussion,
  GitHubWebhookForkee,
  GitHubWebhookHeadCommit,
  GitHubWebhookIssue,
  GitHubWebhookMember,
  GitHubWebhookRelease,
  GitHubWebhookReview,
  GlobalSearchMessagesRequest,
  GroupDmInviteMetadataResponse,
  GroupDmInviteResponse,
  GuildAdminResponse,
  GuildAssetItemSchema,
  GuildAuditLogEntryResponse,
  GuildAuditLogEntryResponseOptions,
  GuildAuditLogListResponse,
  GuildBanCreateRequest,
  GuildBanResponse,
  GuildCreateRequest,
  GuildDeleteRequest,
  GuildEmojiBulkCreateRequest,
  GuildEmojiBulkCreateResponse,
  GuildEmojiBulkCreateResponseFailedInner,
  GuildEmojiCreateRequest,
  GuildEmojiResponse,
  GuildEmojiUpdateRequest,
  GuildEmojiWithUserResponse,
  GuildExplicitContentFilter,
  GuildInviteMetadataResponse,
  GuildInviteMetadataResponseGuild,
  GuildInviteResponse,
  GuildMFALevel,
  GuildMemberResponse,
  GuildMemberSearchRequest,
  GuildMemberSearchResponse,
  GuildMemberSearchResult,
  GuildMemberSearchResultSupplemental,
  GuildMemberUpdateRequest,
  GuildMemoryStatsResponse,
  GuildMemoryStatsResponseGuildsInner,
  GuildReportCategoryEnum,
  GuildResponse,
  GuildRoleCreateRequest,
  GuildRoleHoistPositionItem,
  GuildRolePositionItem,
  GuildRoleResponse,
  GuildRoleUpdateRequest,
  GuildStickerBulkCreateRequest,
  GuildStickerBulkCreateResponse,
  GuildStickerBulkCreateResponseFailedInner,
  GuildStickerCreateRequest,
  GuildStickerResponse,
  GuildStickerUpdateRequest,
  GuildStickerWithUserResponse,
  GuildTransferOwnershipRequest,
  GuildUpdateRequest,
  GuildUpdateResponse,
  GuildUpdateResponseGuild,
  GuildVanityURLResponse,
  GuildVanityURLUpdateRequest,
  GuildVanityURLUpdateResponse,
  GuildVerificationLevel,
  HandoffCompleteRequest,
  HandoffInitiateResponse,
  HandoffStatusResponse,
  HarvestCreationResponseSchema,
  HarvestDownloadUrlResponse,
  HarvestStatusEnum,
  HarvestStatusResponseSchema,
  IndexRefreshStatusResponse,
  IndexRefreshStatusResponseOneOf,
  IndexRefreshStatusResponseOneOf1,
  InstanceConfigResponse,
  InstanceConfigUpdateRequest,
  InstanceConfigUpdateRequestSso,
  InviteMetadataResponseSchema,
  InviteResponseSchema,
  IpAuthorizationPollResponse,
  IpBanListEntrySchema,
  JoinSourceType,
  KickGuildMemberRequest,
  KlipyCategoryTagResponse,
  KlipyFeaturedResponse,
  KlipyGifResponse,
  KlipyRegisterShareRequest,
  LegalHoldRequest,
  LegalHoldResponse,
  LimitConfigGetResponse,
  LimitConfigGetResponseBoundsValue,
  LimitConfigGetResponseLimitConfig,
  LimitConfigGetResponseLimitConfigRulesInner,
  LimitConfigGetResponseLimitConfigRulesInnerFilters,
  LimitConfigGetResponseMetadataValue,
  LimitConfigUpdateRequest,
  LimitConfigUpdateRequestLimitConfig,
  LimitConfigUpdateRequestLimitConfigRulesInner,
  LimitFilterResponse,
  LimitRuleResponse,
  ListAdminApiKeyResponse,
  ListArchivesRequest,
  ListArchivesResponseSchema,
  ListAuditLogsRequest,
  ListBansRequest,
  ListEmailBansResponseSchema,
  ListGuildEmojisResponse,
  ListGuildMembersRequest,
  ListGuildMembersResponse,
  ListGuildStickersResponse,
  ListIpBansResponseSchema,
  ListPhoneBansResponseSchema,
  ListReportsRequest,
  ListReportsResponse,
  ListSnowflakeReservationsResponse,
  ListSystemDmJobsResponse,
  ListUserChangeLogRequest,
  ListUserChangeLogResponseSchema,
  ListUserDmChannelsRequest,
  ListUserDmChannelsResponse,
  ListUserGuildsRequest,
  ListUserGuildsResponse,
  ListUserSessionsRequest,
  ListUserSessionsResponse,
  ListVisionarySlotsResponse,
  ListVoiceRegionsRequest,
  ListVoiceRegionsResponse,
  ListVoiceServersRequest,
  ListVoiceServersResponse,
  Locale,
  LoginRequest,
  LogoutAuthSessionsRequest,
  LookupGuildRequest,
  LookupGuildResponse,
  LookupGuildResponseGuild,
  LookupGuildResponseGuildChannelsInner,
  LookupGuildResponseGuildRolesInner,
  LookupMessageByAttachmentRequest,
  LookupMessageRequest,
  LookupMessageResponse,
  LookupMessageResponseMessagesInner,
  LookupMessageResponseMessagesInnerAttachmentsInner,
  LookupUserByIdsRequest,
  LookupUserByQueryRequest,
  LookupUserRequest,
  LookupUserResponse,
  MessageAckRequest,
  MessageAttachmentResponse,
  MessageAuthorType,
  MessageBaseResponseSchema,
  MessageCallResponse,
  MessageContentType,
  MessageEmbedChildResponse,
  MessageEmbedResponse,
  MessageEmbedType,
  MessageReactionResponse,
  MessageReactionResponseEmoji,
  MessageReferenceResponse,
  MessageReferenceType,
  MessageReportCategoryEnum,
  MessageResponseSchema,
  MessageSearchIndexingResponse,
  MessageSearchResponse,
  MessageSearchResultsResponse,
  MessageSearchResultsResponseMessagesInner,
  MessageSearchScope,
  MessageShredRequest,
  MessageShredRequestEntriesInner,
  MessageShredResponse,
  MessageShredStatusNotFoundResponse,
  MessageShredStatusProgressResponse,
  MessageShredStatusRequest,
  MessageShredStatusResponse,
  MessageSnapshotResponse,
  MessageSortField,
  MessageSortOrder,
  MessageStickerResponse,
  MfaBackupCodeResponse,
  MfaBackupCodesRequest,
  MfaBackupCodesResponse,
  MfaSmsRequest,
  MfaTicketRequest,
  MfaTotpRequest,
  MutualGuildResponse,
  MyGuildMemberUpdateRequest,
  NSFWLevel,
  NcmecSubmissionStatusResponse,
  NcmecSubmitResultResponse,
  NodeStatsResponse,
  NodeStatsResponseMemory,
  OAuth2AuthorizationResponse,
  OAuth2AuthorizationResponseApplication,
  OAuth2ConsentResponse,
  OAuth2IntrospectResponse,
  OAuth2MeResponse,
  OAuth2MeResponseApplication,
  OAuth2MeResponseUser,
  OAuth2TokenResponse,
  OAuth2UserInfoResponse,
  OkResponse,
  PackCreateRequest,
  PackDashboardResponse,
  PackDashboardResponseEmoji,
  PackDashboardResponseSticker,
  PackInviteCreateRequest,
  PackInviteMetadataResponse,
  PackInviteMetadataResponsePack,
  PackInviteResponse,
  PackSummaryResponse,
  PackUpdateRequest,
  PasswordChangeCompleteRequest,
  PasswordChangeStartResponse,
  PasswordChangeTicketRequest,
  PasswordChangeVerifyRequest,
  PasswordChangeVerifyResponse,
  PermissionOverwriteCreateRequest,
  PhoneAddRequest,
  PhoneSendVerificationRequest,
  PhoneVerifyRequest,
  PhoneVerifyResponse,
  PreloadMessagesRequest,
  PreloadMessagesResponseValue,
  PriceIdsResponse,
  PurgeGuildAssetErrorSchema,
  PurgeGuildAssetResultSchema,
  PurgeGuildAssetsRequest,
  PurgeGuildAssetsResponseSchema,
  PushSubscribeRequest,
  PushSubscribeRequestKeys,
  PushSubscribeResponse,
  PushSubscriptionItemResponse,
  PushSubscriptionsListResponse,
  ReadStateAckBulkRequest,
  RefreshSearchIndexRequest,
  RefreshSearchIndexResponse,
  RegisterRequest,
  RelationshipNicknameUpdateRequest,
  RelationshipResponse,
  RelationshipTypePutRequest,
  RelationshipTypes,
  ReloadAllGuildsResponse,
  ReloadGuildRequest,
  ReloadGuildsRequest,
  RenderSpoilers,
  ReorderConnectionsRequest,
  ReportAdminResponseSchema,
  ReportAdminResponseSchemaMessageContextInner,
  ReportGuildRequest,
  ReportMessageRequest,
  ReportResponse,
  ReportStatus,
  ReportType,
  ReportUserRequest,
  ReserveVisionarySlotRequest,
  ResetPasswordRequest,
  ResolveReportRequest,
  ResolveReportResponse,
  RtcRegionResponse,
  SaveMessageRequest,
  SavedMessageEntryResponse,
  ScheduleAccountDeletionRequest,
  ScheduledMessageAllowedMentionsSchema,
  ScheduledMessageReferenceSchema,
  ScheduledMessageResponseSchema,
  ScheduledMessageResponseSchemaPayload,
  SearchAuditLogsRequest,
  SearchGuildsRequest,
  SearchGuildsResponse,
  SearchReportsRequest,
  SearchReportsResponse,
  SearchUsersRequest,
  SearchUsersResponse,
  SendPasswordResetRequest,
  SentryWebhook,
  SentryWebhookActor,
  SentryWebhookData,
  SentryWebhookDataIssue,
  SentryWebhookDataIssueMetadata,
  SentryWebhookDataIssueProject,
  SentryWebhookInstallation,
  SetUserAclsRequest,
  SetUserBotStatusRequest,
  SetUserSystemStatusRequest,
  SetUserTraitsRequest,
  ShrinkVisionarySlotsRequest,
  ShutdownGuildRequest,
  SlackWebhookRequest,
  SlackWebhookRequestAttachmentsInner,
  SlackWebhookRequestAttachmentsInnerFieldsInner,
  SnowflakeReservationEntry,
  SsoCompleteRequest,
  SsoCompleteResponse,
  SsoConfigResponse,
  SsoStartRequest,
  SsoStartResponse,
  SsoStatusResponse,
  StickerAnimationOptions,
  StreamPreviewUploadBodySchema,
  StreamUpdateBodySchema,
  SuccessResponse,
  SudoMfaMethodsResponse,
  SudoVerificationSchema,
  SwapVisionarySlotsRequest,
  SystemDmJobResponse,
  TempBanUserRequest,
  TenorCategoryTagResponse,
  TenorFeaturedResponse,
  TenorGifResponse,
  TenorRegisterShareRequest,
  TerminateSessionsRequest,
  TerminateSessionsResponse,
  ThemeCreateRequest,
  ThemeCreateResponse,
  TicketResponse,
  TimeFormatTypes,
  TransferGuildOwnershipRequest,
  TriggerGuildArchiveRequest,
  TriggerUserArchiveRequest,
  UnlinkPhoneRequest,
  UpdateConnectionRequest,
  UpdateFavoriteMemeBodySchema,
  UpdateGuildFeaturesRequest,
  UpdateGuildNameRequest,
  UpdateGuildSettingsRequest,
  UpdateGuildVanityRequest,
  UpdateSuspiciousActivityFlagsRequest,
  UpdateUserFlagsRequest,
  UpdateVoiceRegionRequest,
  UpdateVoiceRegionResponse,
  UpdateVoiceRegionResponseRegion,
  UpdateVoiceServerRequest,
  UpdateVoiceServerResponse,
  UpdateVoiceServerResponseServer,
  UrlResponse,
  UserAdminResponseSchema,
  UserAuthenticatorTypes,
  UserContactChangeLogEntrySchema,
  UserGuildSettingsResponse,
  UserGuildSettingsResponseChannelOverridesValue,
  UserGuildSettingsResponseMuteConfig,
  UserGuildSettingsUpdateRequest,
  UserGuildSettingsUpdateRequestChannelOverridesValue,
  UserGuildSettingsUpdateRequestMuteConfig,
  UserGuildSettingsUpdateRequestMuteConfigEndTime,
  UserMutationResponse,
  UserNoteResponse,
  UserNoteUpdateRequest,
  UserNotificationSettings,
  UserPartialResponse,
  UserPremiumTypes,
  UserPrivateResponse,
  UserPrivateResponsePendingBulkMessageDeletion,
  UserProfileFullResponse,
  UserProfileFullResponseGuildMemberProfile,
  UserProfileFullResponseUser,
  UserProfileFullResponseUserProfile,
  UserReportCategoryEnum,
  UserSessionResponse,
  UserSettingsResponse,
  UserSettingsResponseGuildFoldersInner,
  UserSettingsUpdateRequest,
  UserSettingsUpdateRequestGuildFoldersInner,
  UserSettingsUpdateRequestStatusResetsAt,
  UserStatusType,
  UserTagCheckResponse,
  UserThemeType,
  UserUpdateWithVerificationRequest,
  UsernameSuggestionsRequest,
  UsernameSuggestionsResponse,
  ValidationErrorItem,
  VerifyAndCreateConnectionRequest,
  VerifyEmailRequest,
  VerifyUserEmailRequest,
  VisionarySlotOperationResponse,
  VisionarySlotSchema,
  VoiceRegionAdminResponse,
  VoiceRegionWithServersResponse,
  VoiceServerAdminResponse,
  WebAuthnAuthenticateRequest,
  WebAuthnChallengeResponse,
  WebAuthnCredentialResponse,
  WebAuthnCredentialUpdateRequest,
  WebAuthnMfaRequest,
  WebAuthnRegisterRequest,
  WebhookCreateRequest,
  WebhookReceivedResponse,
  WebhookResponse,
  WebhookTokenResponse,
  WebhookTokenUpdateRequest,
  WebhookType,
  WebhookUpdateRequest,
  WellKnownFluxerResponse,
  WellKnownFluxerResponseAppPublic,
  WellKnownFluxerResponseCaptcha,
  WellKnownFluxerResponseEndpoints,
  WellKnownFluxerResponseFeatures,
  WellKnownFluxerResponseFederation,
  WellKnownFluxerResponseGif,
  WellKnownFluxerResponseLimits,
  WellKnownFluxerResponseOauth2,
  WellKnownFluxerResponsePublicKey,
  WellKnownFluxerResponsePush,
  WellKnownFluxerResponseSso,
])
Serializers serializers = (_$serializers.toBuilder()
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(MessageResponseSchema)]),
        () => ListBuilder<MessageResponseSchema>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltMap, [FullType(String), FullType(String)]),
        () => MapBuilder<String, String>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(KlipyGifResponse)]),
        () => ListBuilder<KlipyGifResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(UserPartialResponse)]),
        () => ListBuilder<UserPartialResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(RelationshipResponse)]),
        () => ListBuilder<RelationshipResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltMap, [FullType(String), FullType(num)]),
        () => MapBuilder<String, num>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(WebAuthnCredentialResponse)]),
        () => ListBuilder<WebAuthnCredentialResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(FavoriteMemeResponse)]),
        () => ListBuilder<FavoriteMemeResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildResponse)]),
        () => ListBuilder<GuildResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ApplicationResponse)]),
        () => ListBuilder<ApplicationResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GiftCodeMetadataResponse)]),
        () => ListBuilder<GiftCodeMetadataResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ListAdminApiKeyResponse)]),
        () => ListBuilder<ListAdminApiKeyResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(WebhookResponse)]),
        () => ListBuilder<WebhookResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildRoleHoistPositionItem)]),
        () => ListBuilder<GuildRoleHoistPositionItem>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(SavedMessageEntryResponse)]),
        () => ListBuilder<SavedMessageEntryResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(RtcRegionResponse)]),
        () => ListBuilder<RtcRegionResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildStickerWithUserResponse)]),
        () => ListBuilder<GuildStickerWithUserResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(InviteMetadataResponseSchema)]),
        () => ListBuilder<InviteMetadataResponseSchema>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ConnectionResponse)]),
        () => ListBuilder<ConnectionResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(OAuth2AuthorizationResponse)]),
        () => ListBuilder<OAuth2AuthorizationResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildBanResponse)]),
        () => ListBuilder<GuildBanResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildRolePositionItem)]),
        () => ListBuilder<GuildRolePositionItem>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ScheduledMessageResponseSchema)]),
        () => ListBuilder<ScheduledMessageResponseSchema>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ChannelResponse)]),
        () => ListBuilder<ChannelResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(DiscoveryCategoryResponse)]),
        () => ListBuilder<DiscoveryCategoryResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltMap, [FullType(String), FullType(PreloadMessagesResponseValue)]),
        () => MapBuilder<String, PreloadMessagesResponseValue>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(DiscoveryApplicationResponse)]),
        () => ListBuilder<DiscoveryApplicationResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(ChannelPositionUpdateRequestInner)]),
        () => ListBuilder<ChannelPositionUpdateRequestInner>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildMemberResponse)]),
        () => ListBuilder<GuildMemberResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(AuthSessionResponse)]),
        () => ListBuilder<AuthSessionResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(TenorGifResponse)]),
        () => ListBuilder<TenorGifResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildRoleResponse)]),
        () => ListBuilder<GuildRoleResponse>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(String)]),
        () => ListBuilder<String>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(GuildEmojiWithUserResponse)]),
        () => ListBuilder<GuildEmojiWithUserResponse>(),
      )
      ..add(const OneOfSerializer())
      ..add(const AnyOfSerializer())
      ..add(const DateSerializer())
      ..add(Iso8601DateTimeSerializer())
    ).build();

Serializers standardSerializers =
    (serializers.toBuilder()..addPlugin(StandardJsonPlugin())).build();
